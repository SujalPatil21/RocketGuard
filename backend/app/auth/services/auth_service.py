...
class AuthService:
    ...
    def register_user(self, db: Session, req) -> tuple[User, str]:
        ...
        Username is derived from the email local-part when not explicitly supplied.
        """
        logger.info("Registration Started")

        # Validate password matches confirmation (only when confirm_password provided)
        if req.confirm_password and req.password != req.confirm_password:
            logger.info(f"[AUDIT] Registration Failure: passwords mismatch for email={req.email}")
            raise PasswordsDoNotMatchException()

        # Derive username from email local-part when not supplied
        if not req.username:
            base = req.email.split('@')[0]  # e.g. "sujal" from "sujal@example.com"
            # Keep only alphanumeric + underscore chars
            base = ''.join(c if c.isalnum() or c == '_' else '_' for c in base)[:30]
            candidate = base
            counter = 1
            while self.repo.get_user_by_username(db, candidate):
                candidate = f"{base[:27]}_{counter}"
                counter += 1
            derived_username = candidate
        else:
            derived_username = req.username
        ...
        # Check if derived/supplied username is already taken by a DIFFERENT user
        user_by_username = self.repo.get_user_by_username(db, derived_username)
        if user_by_username and user_by_username.email != req.email:
            # Append counter to make it unique
            counter = 1
            while self.repo.get_user_by_username(db, f"{derived_username}_{counter}"):
                counter += 1
            derived_username = f"{derived_username}_{counter}"
