...
def get_notification_service():
    ...
    Returns ConsoleNotificationService when DEMO_MODE=true (prints OTP to server
    console), or the real SMTP EmailNotificationService otherwise.
    Swapping here is the ONLY change needed — auth_service, OTP crypto, and
    all security logic are untouched.
    """
    from app.core.config import settings
    if settings.DEMO_MODE:
        from app.auth.services.console_notification_service import ConsoleNotificationService
        return ConsoleNotificationService()
    return EmailNotificationService()

def get_auth_service(notifier=Depends(get_notification_service)) -> AuthService:
    """
